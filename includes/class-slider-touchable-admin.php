...
class SliderTouchable_Admin {
	...
	**/
	public function custom_meta_box_wp_slider_touchable(){ 
		...
		jQuery(document).ready(function(){
			...
			console.log(tempJsonSettings.settings);
		});
		...
		<div class="attachments-browser hide-sidebar sidebar-for-errors wp_slider_touchable_container row">
			<div class="media-toolbar wp-filter">
					<div class="media-toolbar-secondary">
						...
						<input type="button" value="Upload Images" id="btn_select_pictures_slider_touchable" class="btn_upload_slider_touchable btn btn-danger"/> 
						<?php 
						if($hasImages==0){
							?>
								<input type="button" value="Upload Images" id="btn_upload_slider_touchable" class="btn_upload_slider_touchable btn btn-danger"/> 
								<input type="button" value="Add Images" id="btn_add_more_pictures_slider_touchable" class="btn_add_more_pictures_slider_touchable btn btn-primary"/>
						<?php 
						} 
						else{
								?>
								<input type="button" value="Select Images" id="btn_add_more_pictures_slider_touchable" class="btn_add_more_pictures_slider_touchable btn btn-primary"/>
								<?php
						}
						?>  
					...
					<div class="tabs-content">
						<div class="tab-pane active tab-content current active" id="slides"> 
							<div class="col-lg-11 slider-ui-subcontainer"> 
										...
										<div class="block__list block__list_words img_slider_touch">
											<ul id="sortable_slides_imgs"> 
												...
												**/
												...
												?>
												<script type="text/javascript" id="slider_touchableObject">
													<?php
													$tempJson="";
													...
													(function(){  
														jQuery(document).ready(function($){
															...
															for (var counter = 0; counter < sliderTouchableImagesJsonObject.length; counter++) {
																...
																'" data-url="'+sliderTouchableImagesJsonObject[counter].url+'" > '+
															...
															jQuery( "#sortable_slides_imgs" ).sortable({ 
																	update: function(event, ui) { 
																		...
																		jQuery("ul#sortable_slides_imgs li.ui-state-default.col-sm-2").each(function(o){
																				...
																				var objUpdated = { 
																						order:parseInt(o+1),
																						id: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("id"),
																						url: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("url"),
																						caption: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("caption"),
																						slideBackground: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("slide-background-caption"),
																						captionBackground: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("background-caption"),
																						title: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("title-caption"),
																						readMoreTxt: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("txt-btn-read-more-caption"),
																						readMoreUrl: jQuery('ul#sortable_slides_imgs li.ui-state-default.col-sm-2:nth-child('+parseInt(o+1)+')').data("url-btn-read-more-caption")
								...
								<?php  
								?>
								<textarea id="slider_touchable_object" name="slider_touchable_object" class="slider_touchable_object"></textarea>
							</div>
						</div>
						...
						<div class="tab-pane tab-content " id="settings">
							...
							<textarea id="slider_touchable_object_settings" name="slider_touchable_object_settings" class="slider_touchable_object_settings" style="display:none;"></textarea>
						</div> 
						...
						jQuery(document).ready(function($){
							...
							$('ul#sortable_slides_imgs li.ui-state-default.col-sm-2').each(function(index){
								var objSliderImages = { 
									...
									captionBackground: $(this).data("background-caption"),
									slideBackground: $(this).data("slide-background-caption"),
							...
							**/
							...
							/**
							* Function for select images on the slider_touchable post_type
							**/
							$('#btn_upload_slider_touchable').on('click',function(e){
									e.preventDefault();
									if(mediaUploader){
										mediaUploader.open();
										return;
									...
									mediaUploader.on('select',function(){
										...
										$('textarea#slider_touchable_object').text(JSON.stringify(imagesData));
							...
							**/
							$('#btn_select_pictures_slider_touchable').on('click',function(e){
									e.preventDefault();
									if(mediaUploader){
										mediaUploader.open();
										return;
							...
							**/
							try {
								tempJson=JSON.parse($('textarea#slider_touchable_object').val());
							} catch (error) {
							...
							setTimeout(() => {
								...
								for (let k = 0; k < tempJson.length; k++) {  
									for (var key in tempJson[k]) {
										//console.log(key + " -> " + tempJson[k][key]); 
							...
							$('.btn-icon-edit').on('click',function(){
								...
								if(tempJson[jsonPosition].caption!==null){//console.log('tempJson[jsonPosition].caption!==null');
									...
									//$('#txt_caption_editor').val(tempJson[jsonPosition].caption);
								}
								else{
									$('#txt_caption').empty();//console.log('tempJson[jsonPosition].caption===null');
									//$('#txt_caption_editor').empty();
								}
								if(tempJson[jsonPosition].readMoreTxt!==null){//console.log('tempJson[jsonPosition].caption!==null');
									$('#txt_read_more').val(tempJson[jsonPosition].readMoreTxt);
								}
								else{
									$('#txt_read_more').empty();
								}
								if(tempJson[jsonPosition].readMoreUrl!==null){//console.log('tempJson[jsonPosition].caption!==null');
									$('#txt_read_more_url').val(tempJson[jsonPosition].readMoreUrl);
								}
								else{
									$('#txt_read_more_url').empty();
								}
								if(tempJson[jsonPosition].title!==null){//console.log('tempJson[jsonPosition].caption!==null');
							...
							$('#btn-save').on('click',function(){
								...
								var tempTitle=$('#txt_title').val();
								var tempCaptionBackground=$('#caption_bg_color_picker').val();
								var tempSlideBackground=$("#slide_bg_color_picker").val();
								var tempReadMoreTxt=$("#txt_read_more").val();
								var tempReadMoreUrl=$("#txt_read_more_url").val();
								console.log('jsonPosition ',jsonPosition);
								if(tempCaption!==''){
								...
								if(tempTitle!==''){
									...
									Object.assign(tempJson[jsonPosition], {title:tempTitle});
								}
								if(tempCaptionBackground!==''){
									...
									Object.assign(tempJson[jsonPosition], {captionBackground:tempCaptionBackground});
								}
								if(tempSlideBackground!==''){
									...
									Object.assign(tempJson[jsonPosition], {slideBackground:tempSlideBackground});
								}
								if(tempReadMoreTxt!==''){
									...
									Object.assign(tempJson[jsonPosition], {readMoreTxt:tempReadMoreTxt});
								}
								if(tempReadMoreUrl!==''){
									...
									Object.assign(tempJson[jsonPosition], {readMoreUrl:tempReadMoreUrl});
								}  
				...
				function editCaption(e){
					try {
						...
						if(tempJson[jsonPosition].caption!==null){//console.log('tempJson[jsonPosition].caption!==null');
							...
							//jQuery('#txt_caption_editor').val(tempJson[jsonPosition].caption)
						}
						else{
							//jQuery('#txt_caption_editor').empty();
							jQuery('#txt_caption').empty();//console.log('tempJson[jsonPosition].caption===null');
						}//console.log(jsonPosition);console.log(tempJson[jsonPosition]);Object.assign(target, source);
	...
	**/
	public function wp_save_slider_touchable_meta_box($post_id, $post, $update){
		if (!isset($_POST["slider_touchable-nonce"]) || 
	...
	public function wp_slider_touchable_init() {
		...
		**/
		...
		$labelsSliderPostType = array(
			...
			//'view_items'          => __( 'View Galleries', 'wp_slider_touchable' ),
			...
			//'attributes'=> __( 'Post Galleries', 'wp_slider_touchable' ),
	...
	**/ 
	public function slider_touchable_shortcodes_posts_columns(){
		global $post;
